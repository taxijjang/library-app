...
class UserServiceTest @Autowired constructor(
    ...
    fun saveUserTest() {
        // given
        val request = UserCreateRequest("김택윤", null)
        ...
        assertThat(results[0].name).isEqualTo("김택윤")
        assertThat(results[0].age).isNull()
        assertThat(results).extracting("name").containsExactly("김택윤")
    }
    ...
    fun getUserLoanHistoriesTest1() {
        // given
        userRepository.save(User("K", null))
        ...
        // then
        assertThat(results).hasSize(1)
        assertThat(results[0].name).isEqualTo("K")
        assertThat(results[0].books).hasSize(0)
    }

    @Test
    @DisplayName("대출 기록이 많은 유저의 정상 동작")
